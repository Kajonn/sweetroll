...
| Bounded rule-action p95 < 500 ms | `sweetroll_http_request_duration_seconds` (`/metrics`), `le="0.5"` bucket | `histogram_quantile(0.95, sum(rate(sweetroll_http_request_duration_seconds_bucket{route="/api/rules/actions"}[5m])) by (le)) < 0.5` |
